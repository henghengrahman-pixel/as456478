...
loadStreams=async function(page=1){return baseLoadStreams(page)};
...
function hubStreamCard(x){
  const t=streamTeams(x),mid=x.match_id||x.id||x.slug||encodeURIComponent(`${t.home.name}-vs-${t.away.name}`),status=streamState(x),kick=Number(x.match_time||x.timestamp||0)*1000,target=(status==='scheduled'&&x.fixtureId&&x.stream_ready===false)?`/detail/${esc(x.fixtureId)}`:`/match/${esc(mid)}`;
  return `<a class="live-hub-card" data-stream-status="${status}" href="${target}"><div class="live-ribbon ${status}">${status==='live'?'● LIVE':status==='finished'?'BERAKHIR':'AKAN DATANG'}</div><div class="live-league">${esc(x.league_name||x.league||'Sepak Bola')}</div><div class="live-hub-teams">${team(t.home)}<strong>${status==='live'?esc((x.score_home??0)+' : '+(x.score_away??0)):'VS'}</strong>${team(t.away)}</div><div class="live-watch">${status==='live'?'▣ Nonton Sekarang':kick?`Mulai ${fmtDate(kick)} • ${fmtTime(kick)}`:'Buka pertandingan'}</div></a>`;
}
function sarangLiveRail(activeId){
  const rows=(state.streams||[]).filter(x=>streamState(x)==='live');
  if(!rows.length)return '';
  return `<section class="match-live-rail"><a href="/livescore?tab=live">Live Stream Bola ›</a><div>${rows.map(x=>{const t=streamTeams(x),mid=x.match_id||x.id,score=x.score_home==null?'LIVE':`${x.score_home} : ${x.score_away??0}`;return `<a class="mini-live-card ${String(mid)===String(activeId)?'active':''}" href="/match/${esc(mid)}"><small>● LIVE</small><span>${esc(x.league_name||'Sepak Bola')}</span><div>${team(t.home)}<strong>${esc(score)}</strong>${team(t.away)}</div><b>Nonton Sekarang</b></a>`}).join('')}</div></section>`;
}
async function clientUpcoming24h(){
  const now=Date.now(),max=now+24*3600000,dates=[wibDateValue(0),wibDateValue(1)];
  const settled=await Promise.allSettled([...new Set(dates)].map(date=>loadFixtures('date='+date)));
  const fixtures=[...new Map(settled.flatMap(r=>r.status==='fulfilled'?r.value:[]).map(x=>[x.fixture?.id,x])).values()];
  return fixtures.filter(x=>{const t=new Date(x.fixture?.date||0).getTime(),s=String(x.fixture?.status?.short||'NS');return Number.isFinite(t)&&t>=now-5*60000&&t<=max&&!['FT','AET','PEN','CANC','PST','ABD','AWD','WO'].includes(s)}).map(x=>({id:'schedule-'+x.fixture.id,match_id:'schedule-'+x.fixture.id,fixtureId:String(x.fixture.id),stream_ready:false,stream_provider:'api-football-schedule',league_id:x.league?.id||'',league_name:x.league?.name||'Sepak Bola',home_team_name:x.teams?.home?.name||'Home',home_team_logo:x.teams?.home?.logo||'',away_team_name:x.teams?.away?.name||'Away',away_team_logo:x.teams?.away?.logo||'',score_home:x.goals?.home??null,score_away:x.goals?.away??null,elapsed:null,match_status:'upcoming',match_time:Math.floor(t/1000),title:(x.teams?.home?.name||'Home')+' vs '+(x.teams?.away?.name||'Away')}));
}
function mergeHubStreams(primary,fallback){
  const key=x=>streamPairKey(streamTeams(x).home.name,streamTeams(x).away.name),map=new Map();
  for(const x of [...fallback,...primary]){const k=key(x);if(!k)continue;const prev=map.get(k);if(!prev||x.stream_ready||streamState(x)==='live')map.set(k,x)}
  return [...map.values()].sort((a,b)=>{const sa=streamState(a)==='live'?0:1,sb=streamState(b)==='live'?0:1;return sa-sb+(Number(a.match_time||0)-Number(b.match_time||0))});
}
async function liveStreamHub(){
  app.innerHTML='<div class="loading"><span></span> Memuat pertandingan live...</div>';
  try{
    const [provider,scheduled]=await Promise.all([loadStreams().catch(()=>[]),clientUpcoming24h().catch(()=>[])]),streams=mergeHubStreams(provider,scheduled);state.streams=streams;
...
scoreRow=function(x,streamPairs){
  ...
  return `<a class="score-row" href="${target}"><div class="score-meta"><b>${esc(status)}</b><span>${fmtTime(f.date)}</span></div><div class="score-team home-team"><img src="${esc(h.logo||'/favicon.svg')}"><b>${esc(h.name)}</b></div><strong class="score-pill">${score}</strong><div class="score-team"><img src="${esc(a.logo||'/favicon.svg')}"><b>${esc(a.name)}</b></div>${hasStream?'<span class="live-btn">▣ Live</span>':'<span></span>'}</a>`;
};
...
function videoOverlayAd(matchId){const ad=adFor('overlay',matchId);if(!ad?.media)return;const delay=Math.max(0,Number(ad.delay||3))*1000,set=()=>{const box=document.querySelector('#ad');if(!box)return;box.className='ad-overlay connected-video-ad';box.innerHTML=`<a href="${esc(ad.clickUrl||'#')}" target="_blank">${ad.type==='video'?`<video src="${esc(ad.media)}" autoplay muted playsinline></video>`:`<img src="${esc(ad.media)}">`}<h3>${esc(ad.title||'Iklan')}</h3></a><button class="ad-close btn" disabled>Lewati dalam <b>5</b> detik</button>`;let left=5;const timer=setInterval(()=>{left--;const btn=box.querySelector('.ad-close');if(!btn)return clearInterval(timer);if(left<=0){clearInterval(timer);btn.disabled=false;btn.textContent='Lewati iklan ×';btn.onclick=()=>box.remove()}else btn.innerHTML=`Lewati dalam <b>${left}</b> detik`},1000);setTimeout(()=>box.remove(),Math.max(10,Number(ad.duration||20))*1000)};setTimeout(set,delay)}
