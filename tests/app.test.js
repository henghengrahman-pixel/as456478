...
test('RapidAPI streams are football-only and automatically linked to fixture details',async t=>{
  ...
  const resolvedByFixture=await fetch(base+'/api/stream/resolve/9001').then(r=>r.json());assert.equal(resolvedByFixture.ok,true);assert.equal(resolvedByFixture.match.fixtureId,'9001');
});

test('unmatched RapidAPI stream uses Live Football API as detail fallback',async t=>{
  const fake=http.createServer((req,res)=>{res.setHeader('content-type','application/json');if(req.url.startsWith('/fixtures'))return res.end(JSON.stringify({response:[]}));if(req.url.startsWith('/api/v1/matches'))return res.end(JSON.stringify({success:true,data:{matches:[{id:'777',kickoff:'13:00',status:{state:'inplay',display:'55'},league:{id:'vic',name:'Victoria NPL'},home:{id:'oak',name:'Oakleigh Cannons',logo:'oak.png',score:1},away:{id:'dan',name:'Dandenong City SC',logo:'dan.png',score:0}}]}}));if(req.url.startsWith('/api/v1/live_match_details'))return res.end(JSON.stringify({success:true,data:{events:[{time:'12',side:'home',type:'Goal',detail:{player:{name:'Player A'},score:'1-0'}}],stats:[{label:'Shots',home:8,away:3}]}}));if(req.url.startsWith('/matches'))return res.end(JSON.stringify({matches:[{match_id:'oak-live',sport:'football',status:'live',league_name:'Victoria NPL',home_team_name:'Oakleigh Cannons',away_team_name:'Dandenong City SC',timestamp:1787922000,stream_url:'https://video.test/oak.m3u8'}]}));res.statusCode=404;res.end('{}')});
  await new Promise(resolve=>fake.listen(0,'127.0.0.1',resolve));t.after(()=>fake.close());
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'bola-lfa-')),port=21000+Math.floor(Math.random()*1000),fakePort=fake.address().port,origin=`http://127.0.0.1:${fakePort}`;
  const proc=spawn(process.execPath,['server.js'],{cwd:process.cwd(),env:{...process.env,PORT:String(port),DATA_DIR:dir,JWT_SECRET:'test-secret-value-123456789',ADMIN_PASSWORD:'TestingPassword123',FOOTBALL_API_KEY:'football-key',FOOTBALL_API_BASE:origin,LIVE_FOOTBALL_API_KEY:'live-key',LIVE_FOOTBALL_API_BASE:origin+'/api/v1',RAPIDAPI_KEY:'rapid-key',STREAM_API_HOST:'fake.test',STREAM_API_BASE:origin,STREAM_MATCHES_PATH:'/matches'}});t.after(()=>proc.kill());
  await new Promise((resolve,reject)=>{const end=Date.now()+8000;(function ping(){fetch(`http://127.0.0.1:${port}/health`).then(r=>r.ok&&resolve()).catch(()=>{});if(Date.now()>end)return reject(Error('server timeout'));setTimeout(ping,100)})()});
  const base=`http://127.0.0.1:${port}`,streams=await fetch(base+'/api/streams').then(r=>r.json());assert.equal(streams.matches.length,1);assert.equal(streams.matches[0].fixtureId,'lfa-777');assert.equal(streams.matches[0].score_home,1);assert.equal(streams.matches[0].score_away,0);
  const fixture=await fetch(base+'/api/football/fixtures?id=lfa-777').then(r=>r.json());assert.equal(fixture.response[0].teams.home.name,'Oakleigh Cannons');
  const events=await fetch(base+'/api/fixture/lfa-777/events').then(r=>r.json());assert.equal(events.response[0].type,'Goal');
});
