...
test('server, auth, admin, stream filtering, ads and public pages',async t=>{
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'omtogel-'));
  ...
  for(const route of ['/','/livescore','/odds','/prediksi','/klasemen','/berita','/login','/register','/admin','/match/example']){
    const response=await fetch(base+route);
    assert.equal(response.status,200,route);
    assert.match(await response.text(),/OMTOGEL/);
  }
  ...
  const settings=await fetch(base+'/api/admin/settings',{method:'PUT',headers,body:JSON.stringify({siteName:'OMTOGEL'})}).then(r=>r.json());
  assert.equal(settings.ok,true);
  await new Promise(r=>setTimeout(r,180));
  assert.equal(fs.existsSync(path.join(dir,'omtogel.json')),true);
});
...
test('external odds adapter normalizes and matches fixture safely',async t=>{
  const provider=http.createServer(async(req,res)=>{
    ...
    res.end(JSON.stringify({data:{events:[
      {fixtureId:'777',leagueName:'Premier League',homeName:'Arsenal',awayName:'Chelsea',homeOdds:'1.82',drawOdds:'3.55',awayOdds:'4.10',handicapLine:'-0.5',handicapHomeOdds:'1.91',handicapAwayOdds:'1.95',totalLine:'2.5',overOdds:'1.88',underOdds:'1.96'}
    ]}}));
  });
  await new Promise(resolve=>provider.listen(0,'127.0.0.1',resolve));
  t.after(()=>provider.close());
  const providerPort=provider.address().port;
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'omtogel-odds-'));
  const port=23000+Math.floor(Math.random()*1000);
  const proc=spawn(process.execPath,['server.js'],{cwd:process.cwd(),env:{...process.env,PORT:String(port),DATA_DIR:dir,JWT_SECRET:'test-secret-value-123456789',ADMIN_PASSWORD:'TestingPassword123',FOOTBALL_API_KEY:'',LIVE_FOOTBALL_API_KEY:'',RAPIDAPI_KEY:'',ODDS_API_URL:`http://127.0.0.1:${providerPort}/api/SpreadSetting/GetMoneyLineMappingOddsList`,ODDS_API_METHOD:'POST',ODDS_BEARER_TOKEN:'odds-test-token',ODDS_API_BODY:'{}',ODDS_CACHE_SECONDS:'15'}});
  t.after(()=>proc.kill());
  await new Promise((resolve,reject)=>{const end=Date.now()+8000;(function ping(){fetch(`http://127.0.0.1:${port}/health`).then(r=>r.ok&&resolve()).catch(()=>{});if(Date.now()>end)return reject(Error('server timeout'));setTimeout(ping,100)})()});
  const base=`http://127.0.0.1:${port}`;
  const health=await fetch(base+'/health').then(r=>r.json());
  assert.equal(health.oddsReady,true);
  const status=await fetch(base+'/api/odds/status').then(r=>r.json());
  assert.equal(status.ok,true);
  assert.equal(status.count>=1,true);
  const one=await fetch(base+'/api/odds/match?fixtureId=777&home=Arsenal&away=Chelsea').then(r=>r.json());
  assert.equal(one.ok,true);
  assert.equal(one.match.home,'Arsenal');
  assert.equal(one.match.markets['1x2'].home,1.82);
  assert.equal(one.match.markets.handicap.line,'-0.5');
  assert.equal(one.match.markets.overUnder.over,1.88);
});
