...
test('RapidAPI streams are football-only and automatically linked to fixture details',async t=>{
  ...
  const fake=http.createServer((req,res)=>{res.setHeader('content-type','application/json');if(req.url.startsWith('/fixtures'))return res.end(JSON.stringify({response:[fixture]}));res.end(JSON.stringify({data:{matches:[
